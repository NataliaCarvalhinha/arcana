...
assert.match(worker, /arcana-shell-v24/, "service worker cache version invalidates old app shell");
...
assert.match(app, /function markStudyItemIncomplete/, "completion undo is centralized");
assert.match(app, /source:"completion-undo"/, "completion undo records an activity log entry");
assert.match(app, /markObsidianPending\("completion_undo"\)/, "completion undo only marks Obsidian for a non-destructive sync");
assert.match(index, /id="focusUndoBtn"/, "focus dialog exposes completion undo");
assert.match(app, /confirmUndoCompletion\(/, "completion undo requires an explicit confirmation path");
...
await vm.runInContext(`(async()=>{
  ...
  state=normalize({
    ...structuredClone(DEFAULT_STATE),
    tracks:[{id:"track-undo",name:"Undo",sigil:"U",subtitle:"",description:"",weeklyGoal:60,progression:"sequential"}],
    activeTrack:"track-undo",
    items:[
      {id:"course-undo-1",kind:"course",track:"track-undo",title:"Course Undo 1",estimatedMinutes:60,progress:100,done:true,status:"concluido",completedAt:"2026-08-17T10:00:00.000Z",modules:[{id:"module-undo-1",title:"Module Undo",progress:100,done:true,status:"concluido",completedAt:"2026-08-17T10:00:00.000Z",lessons:[{id:"lesson-undo-1",title:"Lesson kept",progress:100,done:true,status:"concluido",completedAt:"2026-08-17T09:00:00.000Z",notes:"keep notes",fichamentos:["fic"],focusDraftNoteId:"draft-1"},{id:"lesson-undo-2",title:"Lesson undone",progress:100,done:true,status:"concluido",completedAt:"2026-08-17T10:00:00.000Z",notes:"keep lesson notes",fichamentos:["fic-2"],sessions:[{id:"lesson-session"}]}]}]},
      {id:"course-undo-2",kind:"course",track:"track-undo",title:"Course Undo 2",estimatedMinutes:60,progress:40,status:"em_andamento",important:true,urgent:true,modules:[]}
    ],
    playlists:[{id:"playlist-undo",name:"Playlist Undo",url:"https://www.youtube.com/playlist?list=PLundo123",youtubePlaylistId:"PLundo123",lastSyncAt:null,lastSyncError:null}],
    activePlaylist:"playlist-undo",
    youtubeQueue:[{id:"yt-undo",videoId:"yt-undo",playlistId:"playlist-undo",kind:"youtube",title:"Video Undo",url:"https://www.youtube.com/watch?v=yt-undo",channel:"Canal",thumbnail:"",estimatedMinutes:15,progress:100,status:"concluido",notes:"keep video notes",fichamentos:["yt-fic"],createdAt:"2026-08-17T08:00:00.000Z"}],
    sessions:[{id:"session-kept",minutes:25,title:"Historical"}],
    activityLog:[{id:"activity-kept",type:"study",title:"Historical",durationMinutes:25,startedAt:"2026-08-17T08:00:00.000Z",source:"session",sourceRecordId:"session-kept"}],
    weeklyProgress:{"track-undo":90},
    xp:120,
    streak:4,
    dailyPlan:{date:dayKey(),items:[{id:"lesson-undo-2",type:"lesson",status:"done"},{id:"yt-undo",type:"youtube",status:"done"}]}
  });
  const beforeHistory=JSON.stringify({sessions:state.sessions,weeklyProgress:state.weeklyProgress,xp:state.xp,streak:state.streak,lessonNotes:resourceByScope("lesson-undo-2","lesson").notes,videoNotes:resourceByScope("yt-undo","youtube").notes});
  const lessonUndo=markStudyItemIncomplete("lesson-undo-2","lesson",{at:"2026-08-17T12:00:00.000Z"});
  const undoneLesson=resourceByScope("lesson-undo-2","lesson"),undoModule=resourceByScope("module-undo-1","module"),undoCourse=state.items.find(item=>item.id==="course-undo-1"),futureUndoCourse=state.items.find(item=>item.id==="course-undo-2");
  if(!lessonUndo.changed||undoneLesson.progress!==0||undoneLesson.done!==false||undoneLesson.status!=="nao_iniciado"||undoneLesson.completedAt!==null){throw new Error("lesson undo should only reset completion state")}
  if(undoneLesson.notes!=="keep lesson notes"||undoneLesson.fichamentos.length!==1||undoneLesson.sessions.length!==1){throw new Error("lesson undo should preserve notes, fichamentos, and historical sessions")}
  if(undoModule.progress!==50||undoModule.done!==false||undoModule.status!=="em_andamento"||undoCourse.progress!==50||undoCourse.done!==false||undoCourse.status!=="em_andamento"){throw new Error("lesson undo should recalculate module and course progress")}
  if(activeCourseForTrack("track-undo").id!=="course-undo-1"||futureUndoCourse.progress!==40){throw new Error("lesson undo should restore the earliest incomplete active course and preserve future progress")}
  if(findDailyPlanItem("lesson-undo-2","lesson").status!=="pending"){throw new Error("lesson undo should reopen an existing daily plan item")}
  const lessonUndoLog=state.activityLog.find(entry=>entry.source==="completion-undo"&&entry.sourceId==="lesson-undo-2");
  if(!lessonUndoLog||lessonUndoLog.metadata.previousStatus!=="concluido"||lessonUndoLog.metadata.newStatus!=="nao_iniciado"||JSON.stringify(lessonUndoLog).includes("keep lesson notes")){throw new Error("lesson undo should log only small status metadata")}
  const afterLessonHistory=JSON.stringify({sessions:state.sessions,weeklyProgress:state.weeklyProgress,xp:state.xp,streak:state.streak,lessonNotes:undoneLesson.notes,videoNotes:resourceByScope("yt-undo","youtube").notes});
  if(afterLessonHistory!==beforeHistory){throw new Error("lesson undo should preserve study history, XP, streak, and notes")}
  const videoUndo=markStudyItemIncomplete("yt-undo","youtube",{at:"2026-08-17T12:05:00.000Z"});
  const undoneVideo=resourceByScope("yt-undo","youtube");
  if(!videoUndo.changed||undoneVideo.progress!==0||undoneVideo.status!=="nao_iniciado"||undoneVideo.notes!=="keep video notes"||undoneVideo.fichamentos.length!==1){throw new Error("YouTube undo should reset only completion status")}
  if(findDailyPlanItem("yt-undo","youtube").status!=="pending"){throw new Error("YouTube undo should reopen an existing daily plan item")}
  const videoUndoLog=state.activityLog.find(entry=>entry.source==="completion-undo"&&entry.sourceId==="yt-undo");
  if(!videoUndoLog||videoUndoLog.metadata.previousProgress!==100||videoUndoLog.metadata.newProgress!==0||JSON.stringify(videoUndoLog).includes("keep video notes")){throw new Error("YouTube undo should log only status metadata")}
  state=structuredClone(seededFresh);
