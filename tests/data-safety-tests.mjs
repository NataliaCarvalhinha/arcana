...
const undoHelper = app.slice(app.indexOf("function markStudyItemIncomplete"), app.indexOf("function updateFocusUndoButton"));
...
assert.match(app, /function markStudyItemIncomplete/, "completion undo is centralized in one mutation helper");
assert.match(app, /source:"completion-undo"/, "completion undo creates a small audit-log entry");
assert.doesNotMatch(undoHelper, /state\.sessions\s*=/, "completion undo must not rewrite historical sessions");
assert.doesNotMatch(undoHelper, /delete\s+.*(?:notes|fichamentos|blocks|concepts|questions|permanent)/, "completion undo must not delete study artifacts");
assert.match(app, /markObsidianPending\("completion_undo"\)/, "completion undo must not delete Obsidian data and only marks sync pending");
